...
## source: https://github.com/TiredSounds/cmus-conky/blob/master/cmus-conky-art.sh
...
if cmus-remote -Q &>/dev/null ; then
    ...
    # Look for album art, in order of the chance of an error
    # 1. Find an image matching a string of common names. Add more as you wish using '\|foo.jpg' as the structure.
    FILE=$( ls "$FOLDER" | grep -i -m1 'folder.jpg\|cover.jpg\|front.jpg\|folder.png\|cover.png\|front.png' )

    # 2. Remove the

	# 3. Look for any image file in the album folder.
	...
	if [ -z "$FILE" ]; then
        FILE=$( ls "$FOLDER" | grep -i -P -m1 '*.(jpg|png)' )
	fi
