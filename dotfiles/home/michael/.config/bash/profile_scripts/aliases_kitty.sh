...
function verify_byte_decimal() {
    if echo $1 | grep -Pe '^([0-9]{1,2}|[0-2][0-5]{2})$' >& /dev/null; then
        return 0
    fi
...
function verify_rgb() {
    if ! verify_byte_decimal $1 || ! verify_byte_decimal $2 || \
        ! verify_byte_decimal $3; then
        return 1
    fi
...
# Font style - see https://en.wikipedia.org/wiki/ANSI_escape_code
