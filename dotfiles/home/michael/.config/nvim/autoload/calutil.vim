...
" Date:     Nov 11, 2014h
" Version:  3c  ASTRO-ONLY
" ---------------------------------------------------------------------
if exists("loaded_calutil")
 finish
endif
let g:loaded_calutil= "v3c"
if v:version < 700
 echohl WarningMsg
...
" ---------------------------------------------------------------------
" calutil#Date2Jul: converts a MDY string to a Julian date {{{1
"    Ex. Jan 31, 2014
"        jan 31, 2014
"        january 31, 2014
...
"       "dmy"  : day month year
"       "ymd"  : year month day
fun! calutil#Date2Jul(datestr,...)
"  call Dfunc("calutil#Date2Jul()")

  if a:0 > 0
   if a:1 == 'mdy'
    " Jan 31, 2014   (month day, year)
    let month= substitute(a:datestr,'^\s*\(\a\+\)\s\+\(\d\+\)\s*,\s*\(\d\+\).*$','\1','')
    let day  = substitute(a:datestr,'^\s*\(\a\+\)\s\+\(\d\+\)\s*,\s*\(\d\+\).*$','\2','') + 0
    let year = substitute(a:datestr,'^\s*\(\a\+\)\s\+\(\d\+\)\s*,\s*\(\d\+\).*$','\3','') + 0
   elseif a:1 == 'ymd'
    " 2014 Jan 31   (year month day)
    let year = substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\1','') + 0
    let month= substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\2','')
    let day  = substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\3','') + 0
   elseif a:1 == 'dmy'
    " 31 Jan 2014  (day month year)
    let day  = substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\1','') + 0
    let month= substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\2','')
    let year = substitute(a:datestr,'^\s*\(\d\+\)\s\+\(\a\+\)\s\+\(\d\+\).*$','\3','') + 0
   endif
  else
    " Jan 31, 2014   (month day, year)
   let month= substitute(a:datestr,'^\s*\(\a\+\)\s\+\(\d\+\)\s*,\s*\(\d\+\).*$','\1','')
  ...
  let im= 1
  for mpat in s:months
"   call Decho("im=".im." month<".month."> mpat<".mpat.">")
   if month =~ mpat
    break
   endif
   let im = im + 1
  endfor
"  call Decho("datestr<".a:datestr."> is ".im."/".day."/".year)

  let julday= calutil#Cal2Jul(year,im,day)

"  call Dret("calutil#Date2Jul ".julday)
  return julday
endfun
